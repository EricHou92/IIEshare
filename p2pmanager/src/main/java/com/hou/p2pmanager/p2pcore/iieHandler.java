 ...
 * 所有的message中转的handler，可以接受来自UI或者thread的message，
 * 也可以转发message到UI
 */
public class iieHandler extends Handler{

    private static final String tag = iieHandler.class.getSimpleName();

    private P2PManager p2PManager;
    private iieCommunicate p2PCommunicate;
    private iieManager neighborManager;
    private ReceiveManager receiveManager;
    private SendManager sendManager;

    public iieHandler(Looper looper)
    {
        super(looper);
    }

    public iieManager getNeighborManager()
    {
    ...
    public void init(P2PManager manager, Context context)
    {
        ...
        p2PCommunicate = new iieCommunicate(p2PManager, this, context);
        p2PCommunicate.start();

        neighborManager = new iieManager(p2PManager, this, p2PCommunicate);
        new Thread()
        {
            public void run()
            {
